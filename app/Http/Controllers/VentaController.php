...
class VentaController extends Controller
{
    ...
    public function store(VentaForRequest $request){
        ...
        $clientes=DB::table('cliente')->where('activo','=','1')->get();
        return view('detalle.index',["venta"=>venta::findOrFail($id_venta),'productos'=>$productosCarrito,'id_venta'=>$id_venta,'disponibles'=>$producto,'clientes'=>$clientes]); 
    }
    public function show($id){
        $id_venta=$id;
        $venta=DB::table('ventas as c')
                ->join('cliente as p','p.id_cliente','=','c.id_cliente')
                ->select('c.monto as monto','c.fecha','c.id_venta as folio','p.nombre as nombreP','p.RFC as rfcP','p.telefono as telefonoP','p.e_mail as correoP','p.domicilio as domicilioP')
        ...
        $productos=DB::table('detalle_venta as dv')
                ...
                ->where('dv.id_venta','=',$id_venta)
                ->orderBy('dv.id_detalleventa','desc')->get();
        $view = view ('ventas.show',['venta'=>$venta, 'productos'=>$productos]);
    ...
    public function edit($id){
        ...
        $producto=DB::table('producto as p')
                ...
                ->orderBy('p.id_producto','desc')->get();
        $clientes=DB::table('cliente')->where('activo','=','1')->get();
        return view('ventas.edit',['venta'=>venta::findOrFail($id),'clientes'=>$clientes,'disponibles'=>$producto,'id_venta'=>$id_venta,'productos'=>$productosCarrito]);
    }
    public function update(VentaForRequest $request,$id){
        ...
        $producto=DB::table('producto as p')
                ...
                ->orderBy('p.id_producto','desc')->get();
        $clientes=DB::table('cliente')->where('activo','=','1')->get();
        return view('ventas.edit',['venta'=>venta::findOrFail($id),'clientes'=>$clientes,'productos'=>$productosCarrito,'id_venta'=>$id_venta,'disponibles'=>$producto]);
    }
